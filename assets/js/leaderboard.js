...
function renderLeaderboard(data) {
  ...
  data.forEach((user, index) => {
    ...
    row.innerHTML = `
      ...
      <td>
        <button class="btn btn-sm btn-primary" onclick="viewUserInfo('${user.id}')">
...
function viewUserInfo(userId) {
  ...
  db.collection("data").doc(userId).get().then((doc) => {
    if (doc.exists) {
      ...
      document.getElementById("viewUserName").textContent = user ? user.name || "N/A" : "N/A";
      document.getElementById("viewUserEmail").textContent = user ? user.email || "N/A" : "N/A";
      document.getElementById("viewUserPhone").textContent = "N/A";
    }

    // Points data
    const user = pointsData.find(u => u.id === userId);
    document.getElementById("viewCollectedBins").textContent = user ? user.collectedbins || 0 : 0;
    document.getElementById("viewTotalPoints").textContent = user ? user.totalPoints || 0 : 0;
    document.getElementById("viewLastUpdated").textContent = user && user.timestamp ? user.timestamp.toDate().toLocaleString() : "N/A";
